...
const NewListingCard: React.FC<NewListingCardProps> = ({
  ...
  return (
    <div className="bg-white relative rounded-[12px] shadow-[0px_4px_12px_0px_rgba(0,0,0,0.04)] w-[306px] h-[448px]">
      {/* Image */}
      <div className="absolute h-[200px] left-0 rounded-tl-[12px] rounded-tr-[12px] top-0 w-[306px]">
      ...
      {listing.metroStations && listing.metroStations.length > 0 ? (
        <>
          {listing.metroStations.slice(0, 2).map((station, index) => (
            <div 
              key={index}
              className="absolute flex items-center gap-[8px] left-[16px]"
              style={{ top: `${344 + index * 28}px` }}
            >
              <p className="font-normal leading-[20px] text-[14px] text-gray-900">
                {station.name}
              </p>
              <p className="font-normal leading-[20px] text-[14px] text-gray-700 text-right absolute right-[16px]">
                {station.distance}
              </p>
            </div>
          ))}
