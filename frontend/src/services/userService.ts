...
class UserService {
  ...
  async requestEmailChange(
    newEmail: string
  ): Promise<{ status: string; message: string; new_email: string }> {
    const response = await apiClient.post("/users/email/change-request", null, {
      params: { new_notification_email: newEmail },
    });
    return response.data;
  }
  ...
  async confirmEmailChange(
    ...
    const response = await apiClient.post("/users/email/change-confirm", null, {
      params: { new_notification_email: newEmail, code: code },
    });
    return response.data;
  }
  ...
  async linkTelegramAccount(
    code: string
  ): Promise<{ status: string; message: string; telegram_username: string }> {
