"""
Скрипт для принудительной очистки объявлений из БД (без подтверждения)
"""
import sys
import os
...
def clear_listings():
    ...
    try:
        ...
        print(f"\n🗑️  Удаление {total_before} объявлений...")
        ...
        result = session.execute(text("SELECT COUNT(*) FROM sent_notifications"))
        notifications_before = result.scalar()
        
        session.execute(text("DELETE FROM sent_notifications"))
        session.commit()
        print(f"✅ Очищено уведомлений: {notifications_before}")
