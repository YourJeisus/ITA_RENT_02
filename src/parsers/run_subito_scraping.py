#!/usr/bin/env python3
"""
🚀 СКРИПТ ДЛЯ ЗАПУСКА ПАРСИНГА SUBITO.IT

Парсит главную страницу https://www.subito.it/annunci-lazio/affitto/immobili/roma/roma/
Извлекает все объявления и все фотографии, сохраняет в БД

Использование:
    cd src/parsers
    python run_subito_scraping.py
...
async def main():
    """
    Основная функция запуска парсинга Subito.it
    """
    print("🚀 Парсинг Subito.it (Рим)")
    ...
    scraper = SubitoScraper(enable_geocoding=enable_geocoding)
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Парсинг Subito.it завершен: {len(listings)} объявлений за {execution_time:.1f}с")
